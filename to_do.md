...
| v2.1 resource pass | Bounded caches; lazy pyplot/plotly; news cap 150; vectorized scan filters; leaner GARCH; `requirements.txt` minus pytest/accelerate |
| v2.2 left-panel truncation fix | Wrapping value column + shorter Vol/BB strings + pane min-width |
| v2.2 docs refresh | CLI flags, structure, optional Laya notes, perf notes, template markers |
...
Stock relationship graph, peer basket traversal, spread z-scores, and lead-lag detection live in `core/stock_graph.py` and `sentinel graph`. Optional Laya polarity (`core/laya_decisions.py`, `SENTINEL_LAYA=1`); FinBERT removed.
...
1. FinBERT / torch / transformers removed from requirements; optional Laya is separate.
