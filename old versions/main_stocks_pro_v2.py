...
class MarketApp:
    def __init__(self, root):
        ...
        self.root.geometry("1000x850") # Taller and Wider for Chart
        ...
        self.entry_ticker.bind('<Return>', lambda e: self.load_data()) # Load default 30d
    ...
    def fetch_and_plot(self, ticker, period, interval):
        try:
            ...
            # If we are doing the initial load (1mo), also update the Left Panel Technicals
            # Note: For accurate 14d RSI/ATR, we need at least a few months of daily data.
            # So for the "Technicals Panel", we always fetch 1y daily in background if needed.
            if period == "1mo":
                ...
                self.root.after(0, self.update_technicals, last, hv_30)
    ...
    def update_chart(self, df, ticker, period):
        ...
        # Plot EMAs
        self.ax.plot(df.index, df['EMA_5'], label='EMA 5 (Fast)', color='blue', linewidth=1, linestyle='--')
        self.ax.plot(df.index, df['EMA_21'], label='EMA 21 (Med)', color='orange', linewidth=1)
        self.ax.plot(df.index, df['EMA_63'], label='EMA 63 (Slow)', color='purple', linewidth=1)
    ...
    def update_technicals(self, data, hv):
