...
class MarketApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Technical Command Center Pro V6 (DEBUG MODE)")
        ...
        self.lbl_rsi = self.add_row(self.grid_frame, "RSI (14d)", 0, "RSI Info")
        self.lbl_stoch = self.add_row(self.grid_frame, "Stoch RSI", 1, "Stoch Info")
        self.lbl_macd = self.add_row(self.grid_frame, "MACD", 2, "MACD Info")
        self.lbl_bb = self.add_row(self.grid_frame, "Bollinger Bands", 3, "BB Info")
        self.lbl_atr = self.add_row(self.grid_frame, "ATR (Volatility)", 4, "ATR Info")
        self.lbl_vol = self.add_row(self.grid_frame, "Hist. Vol (30d)", 5, "HV Info")
        self.lbl_sent = self.add_row(self.grid_frame, "News Sentiment", 6, "News Info")
    ...
    # --- DEBUGGING NEW SENTIMENT LOGIC ---
    def get_google_news_rss(self, ticker):
        print(f"\n[DEBUG] Starting Google RSS Fetch for {ticker}...")
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"[DEBUG] RSS Items Found: {len(items)}")
                
                titles = [item.find('title').text for item in items][:10]
                print(f"[DEBUG] First 3 Titles found: {titles[:3]}")
                return titles
            else:
                print(f"[DEBUG] Failed with status code: {resp.status_code}")
        except Exception as e:
            print(f"[DEBUG] RSS Fallback EXCEPTION: {e}")
        return []

    def calculate_sentiment(self, ticker, stock_obj):
        print("\n--- SENTIMENT CHECK START ---")
        
        # Check cache
        if ticker in self.sent_cache:
            val, ts = self.sent_cache[ticker]
            if time.time() - ts < self.SENT_CACHE_DURATION:
                print("[DEBUG] Returning Cached Sentiment")
                return val

        headlines = []
        
        # 1. Try Yahoo
        print("[DEBUG] Attempting yfinance.news...")
        try:
            ynews = stock_obj.news
            print(f"[DEBUG] yfinance returned type: {type(ynews)}")
            if ynews:
                print(f"[DEBUG] yfinance items count: {len(ynews)}")
                headlines = [n.get('title', '') for n in ynews]
            else:
                print("[DEBUG] yfinance returned EMPTY list.")
        except Exception as e:
            print(f"[DEBUG] yfinance error: {e}")

        # 2. Fallback to RSS if empty
        if not headlines:
            print("[DEBUG] Triggering Google RSS Fallback...")
            headlines = self.get_google_news_rss(ticker)

        if not headlines:
            print("[DEBUG] No headlines found from ANY source. Returning None.")
            return None

        # 3. Analyze
        print(f"[DEBUG] Analyzing {len(headlines)} headlines...")
        polarities = []
        for h in headlines:
            if h:
        ...
        avg_poly = sum(polarities) / len(polarities)
        score = (avg_poly + 1) / 2
        print(f"[DEBUG] Final Score: {score:.4f}")
        
        self.sent_cache[ticker] = (score, time.time())
        return score
    ...
    def fetch_and_plot(self, ticker, period, interval):
        try:
            ...
            if df is None:
                print(f"[DEBUG] Fetching Price Data: {ticker} {period}")
            ...
            print(f"[DEBUG] CRITICAL ERROR IN FETCH: {e}")
