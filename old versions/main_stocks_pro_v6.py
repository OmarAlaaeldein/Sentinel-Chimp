...
def check_nltk_data():
    try:
        ...
        print("\n!!!!!!!!!! CRITICAL WARNING !!!!!!!!!!")
        print("TextBlob is missing the 'punkt' tokenizer data.")
        print("The app might freeze or fail calculating sentiment.")
        print("PLEASE RUN THIS COMMAND IN TERMINAL: python -m textblob.download_corpora")
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n")
...
class MarketApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Technical Command Center Pro V8 (TextBlob Restored)")
        ...
        self.lbl_sent = self.add_row(self.grid_frame, "News Sentiment", 6, 
            "TextBlob AI Analysis of recent headlines.\n0 = Bearish\n1 = Bullish\n0.5 = Neutral")
    ...
    def calculate_sentiment(self, ticker, stock_obj):
        ...
        # 3. Analyze with TextBlob
        try:
            print(f"[DEBUG] TextBlob analyzing {len(headlines)} headlines...")
            polarities = []
            for h in headlines:
                if h:
                    blob = TextBlob(h)
            ...
            print("[ERROR] NLTK 'punkt' not found. Please run 'python -m textblob.download_corpora'")
            return None
