    ...
    def load_model(self):
        ...
        try:
            if os.path.exists(self.local_path) and os.listdir(self.local_path):
                ...
                print(f"[System] Downloading model (First Run)...")
...
class MarketApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Technical Command Center V16 (Auto-Refresh Fix)")
    ...
    def init_model_bg(self):
        ...
        # 2. AUTO-REFRESH FIX: Once model is ready, force refresh the view
        # This fixes the issue where the default stock (AMD) stays at 0.5 until clicked
        if sentiment_engine.is_ready:
            self.root.after(500, lambda: self.log("Model Ready. Auto-refreshing data..."))
            self.root.after(1000, self.load_data)
    ...
    def load_data(self):
        ...
        # Only clear cache if ticker actually changed, to support auto-refresh without flickering
    ...
    def calculate_sentiment(self, ticker, stock_obj):
        ...
        try:
            ...
            if ynews:
                ...
                if headlines:
                    self.log(f"Yahoo found {len(headlines)} valid headlines.")
        except: pass
