...
# Force NLTK to look in local directory if it exists (from the fix script)
local_nltk = os.path.join(os.getcwd(), "nltk_data")
if os.path.exists(local_nltk):
    if local_nltk not in nltk.data.path:
        nltk.data.path.insert(0, local_nltk)

# ===================== 1. Sentiment Engine (Hybrid) =====================
POSITIVE_WORDS = {'up', 'surge', 'jump', 'rise', 'gain', 'bull', 'high', 'record', 'beat', 'strong', 'profit', 'growth', 'buy', 'soar', 'green'}
NEGATIVE_WORDS = {'down', 'drop', 'fall', 'sink', 'loss', 'bear', 'low', 'crash', 'weak', 'miss', 'decline', 'sell', 'plummet', 'dip', 'red'}

def get_sentiment_hybrid(text):
    """
    Tries TextBlob first. If NLTK crashes (missing punkt_tab), 
    falls back to Native logic instantly.
    """
    ...
    try:
        ...
        # 2. Fallback if NLTK is broken
        # print("[DEBUG] TextBlob failed (missing data). Using Native fallback.")
        ...
        # Normalize simple score to -1..1 range roughly
        if score > 0: return min(1.0, score * 0.4)
        if score < 0: return max(-1.0, score * 0.4)
        return 0.0
...
class MarketApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Technical Command Center Pro V10 (Hybrid Engine)")
        ...
        self.lbl_rsi = self.add_row(self.grid_frame, "RSI (14d)", 0, "RSI")
        self.lbl_stoch = self.add_row(self.grid_frame, "Stoch RSI", 1, "Stoch")
        self.lbl_macd = self.add_row(self.grid_frame, "MACD", 2, "MACD")
        self.lbl_bb = self.add_row(self.grid_frame, "Bollinger Bands", 3, "Bollinger")
        self.lbl_atr = self.add_row(self.grid_frame, "ATR (Volatility)", 4, "ATR")
        self.lbl_vol = self.add_row(self.grid_frame, "Hist. Vol (30d)", 5, "HV")
        self.lbl_sent = self.add_row(self.grid_frame, "News Sentiment", 6, 
            "Headline Analysis (Hybrid Engine).\n0 = Bearish\n1 = Bullish\n0.5 = Neutral")
    ...
    def calculate_sentiment(self, ticker, stock_obj):
        ...
        # 3. Analyze (Hybrid Engine)
        polarities = []
        for h in headlines:
            if h:
                # This function now safely handles crashes internally
                pol = get_sentiment_hybrid(h)
                polarities.append(pol)
        
        if not polarities: return None
        
        avg_poly = sum(polarities) / len(polarities)
        final_norm = (avg_poly + 1) / 2
        
        self.sent_cache[ticker] = (final_norm, time.time())
        return final_norm
