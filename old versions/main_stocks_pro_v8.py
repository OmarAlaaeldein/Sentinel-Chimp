...
# Suppress SSL warnings for the aggressive fetcher
...
# ===================== 1. Native Sentiment Engine (Backup) =====================
# Used if TextBlob/NLTK crashes
POSITIVE_WORDS = {'up', 'surge', 'jump', 'rise', 'gain', 'bull', 'high', 'record', 'beat', 'strong', 'profit', 'growth', 'buy', 'soar', 'green', 'rally'}
NEGATIVE_WORDS = {'down', 'drop', 'fall', 'sink', 'loss', 'bear', 'low', 'crash', 'weak', 'miss', 'decline', 'sell', 'plummet', 'dip', 'red', 'slump'}
...
def analyze_native(text):
    ...
    # -2 becomes 0.2, 0 becomes 0.5, +2 becomes 0.8
    final = 0.5 + (score * 0.15)
...
class MarketApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Technical Command Center V11 (With Logger)")
        ...
        # --- BOTTOM: SYSTEM LOG ---
        log_frame = ttk.LabelFrame(root, text="System Log (Debug Output)", padding=5)
        ...
        self.log("App Started. Ready.")
        self.root.after(500, self.load_data)

    def log(self, msg):
        """Prints to the bottom text box"""
        timestamp = datetime.now().strftime("%H:%M:%S")
        full_msg = f"[{timestamp}] {msg}\n"
        self.log_box.insert("end", full_msg)
        self.log_box.see("end")
        print(full_msg) # Also print to terminal just in case
    ...
    def get_google_news_rss(self, ticker):
        self.log(f"Trying Google RSS for {ticker}...")
        try:
            url = f"https://news.google.com/rss/search?q={ticker}+stock&hl=en-US&gl=US&ceid=US:en"
            # Verify=False to bypass SSL errors in some environments
            resp = requests.get(url, timeout=5, verify=False) 
            if resp.status_code == 200:
                root = ET.fromstring(resp.content)
                titles = [item.find('title').text for item in root.findall('.//item')][:10]
                self.log(f"Google RSS success: Found {len(titles)} headlines.")
                return titles
            else:
                self.log(f"Google RSS failed. Code: {resp.status_code}")
        except Exception as e:
             self.log(f"Google RSS Error: {e}")
        return []
    ...
    def calculate_sentiment(self, ticker, stock_obj):
        ...
        try:
            ...
            if ynews:
                headlines = [n.get('title', '') for n in ynews]
                self.log(f"Yahoo News found {len(headlines)} headlines.")
            else:
                self.log("Yahoo News returned empty list.")
        except Exception as e:
            self.log(f"Yahoo News fetch error: {e}")
        ...
        if not headlines:
            self.log("ERROR: No headlines found from any source. Sentiment = N/A.")
            return None
        ...
        # Try TextBlob if installed, but fallback safely
        if NLTK_AVAILABLE:
            try:
                # Test one blob
                test = TextBlob("test")
                test.sentiment # Triggers the lookup error if missing
                
                method_used = "TextBlob"
                for h in headlines:
                    polarities.append(TextBlob(h).sentiment.polarity)
                    
            except (LookupError, OSError, Exception) as e:
                self.log(f"TextBlob/NLTK Crash: {e}. Switching to Native Engine.")
                method_used = "Native"
                polarities = [] # Reset
        
        # Native Logic (Backup)
        if method_used == "Native":
            for h in headlines:
                polarities.append(analyze_native(h))
            # Native returns 0..1, map to -1..1 for averaging logic below
            # Actually analyze_native returns 0..1 already.
            # TextBlob returns -1..1.
            # Let's standardize on 0..1 output for the function.
            pass

        if not polarities: return None
        
        # Averaging
        avg_raw = sum(polarities) / len(polarities)
        
        if method_used == "TextBlob":
            # Map -1..1 to 0..1
            final_score = (avg_raw + 1) / 2
        else:
            # Native is already 0..1
            final_score = avg_raw
            
        self.log(f"Sentiment Calculated ({method_used}): {final_score:.2f}")
        self.sent_cache[ticker] = (final_score, time.time())
        return final_score
    ...
    def fetch_and_plot(self, ticker, period, interval):
        try:
            ...
            if df_tech is None:
                self.log("Fetching 1Y daily data for Technicals...")
            ...
            # 3. Sentiment
            self.log("Starting Sentiment Check...")
