...
class MarketApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Technical Command Center Pro V4 (Cached)")
        self.root.geometry("1100x850")

        # --- CACHE ---
        # Structure: { (ticker, period, interval): (dataframe, timestamp_of_fetch) }
        self.cache = {}
        self.CACHE_DURATION = 60 # Seconds before invalidating cache
        ...
        # Cache Indicator (Small label to show if data was from cache)
    ...
    def load_data(self):
        # Clear cache when loading a NEW ticker manually
    ...
    def get_cached_data(self, ticker, period, interval):
    ...
    def save_to_cache(self, ticker, period, interval, df):
        key = (ticker, period, interval)
        self.cache[key] = (df, time.time())
    ...
    def calculate_sentiment(self, stock):
        try:
            ...
            return (avg_polarity + 1) / 2
        except: return None

    def fetch_and_plot(self, ticker, period, interval):
        try:
            # Check Cache
            df, is_cached = self.get_cached_data(ticker, period, interval)
            ...
            if df is None:
                ...
                self.save_to_cache(ticker, period, interval, df)
            ...
            # Technicals Panel (Run logic only if 1mo/1d or if not cached, or just refresh it)
            # For simplicity, we also cache the "Tech Data" separately or just fetch it less often.
            # Here we just fetch it fresh for the panel to ensure price is accurate, 
            # OR we can use the last Close from the chart if it's recent enough.
            
            # To be safe for API limits, let's use the Chart DF if it is 1d interval,
            # otherwise fetch 1y 1d separately (but we should cache that too).
            
            # Simple approach: Fetch Tech Data (1y, 1d) with its own cache key
            df_tech, tech_cached = self.get_cached_data(ticker, "1y", "1d")
            if df_tech is None:
                ...
                self.save_to_cache(ticker, "1y", "1d", df_tech)
            
            last = df_tech.iloc[-1]
            # Override "Price" with the latest from the granular chart if available and newer
            if not df.empty:
            ...
            # Sentiment (Not cached here, but you could)
            sentiment_score = self.calculate_sentiment(stock)

            # Pass 'last' (tech row) but override Close with current_price
            last_copy = last.copy()
